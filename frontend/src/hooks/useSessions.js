...
// ── Private Session Hooks ────────────────────────────────────────────

export const useCreatePrivateSession = () => {
  const result = useMutation({
    mutationKey: ["createPrivateSession"],
    mutationFn: sessionApi.createPrivateSession,
    onError: (error) => toast.error(error.response?.data?.message || "Failed to send invite"),
  });
  return result;
};

export const useJoinByToken = () => {
  const result = useMutation({
    mutationKey: ["joinByToken"],
    mutationFn: sessionApi.joinByToken,
    onError: (error) => toast.error(error.response?.data?.message || "Failed to join session"),
  });
  return result;
};
